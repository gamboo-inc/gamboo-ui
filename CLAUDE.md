...
- **MCP サーバー**: `.mcp.json` で gamboo-ui サーバーが自動接続される（`npm install` 後）。ツール: `get_token` / `get_component` / `check_rule` / `check_html` / `get_rules` / `search`。UI 生成後は `check_html` で自己検証する
