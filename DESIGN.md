...
> 全ルール（98 件）: `design/contracts/rules.json`（machine-readable）
...
MCP ツール `get_token` / `get_component` / `check_rule` / `get_rules` / `search` を使用。Resource は `melta://tokens` / `melta://components` / `melta://rules`（全98件）/ `melta://rules/auto-detectable`（自動検出サブセット）。
