...
- **既知の制約（Codex review）**: `tag-missing-attr` の正規表現は属性値内に生 `>`（`<th title="a>b">`）を含むと誤判定しうる。実 UI では稀（本来 `&gt;`）で S2(cheerio) 化で解消。`attr-lint.ts` にコメント明記済み。

### 2026-05-30 — Q6 実装完了 + Q4 はデータ判断で見送り

**Q6（inline style / `<style>` escape hatch 検知）✅** — H6(D2-02) の「生 CSS による回避」を精密スコープで塞いだ。Codex 補正（inline style は contract で正規使用＝禁止でなく warn）を実データで再検証して判断。

- **方式**: Q5 の attr-lint に check kind を2つ追加（`attr-value-contains` / `tag-present`）。
- **追加2ルール（warn）**: `COLOR_NO_INLINE_STYLE_HARDCODE`（inline style 内の `#`/`rgb`/`hsl` 直書き色） / `PHILOSOPHY_NO_STYLE_BLOCK`（`<style>` ブロック）
- **精密スコープの裏取り**: docs の inline style 619件中 **505件が `var(--...)`（＝トークン体系の正規利用）**。プロパティ直後アンカー正規表現 `(color|background|...)\s*:\s*(#|rgb|hsl)` で、`color:var(--x, #2250df)`（var の hex フォールバック）すら非検知にしつつ `background:#2250df`（真の回避143件相当）だけ捕捉。`<style>` ブロックは既存使用0件でゼロFP。
- **テスト**: `tests/lint-core.spec.ts` に Q6 検知4 + ガード4。全 67 unit test green / tsc 緑 / design:check 緑。
- **注意**: 生成物 docs/index.html には hardcoded color in style が143件あり（showcase 自動生成由来）。warn かつ CI は変更ファイルのみ lint なので実害なし。気になればジェネレータ修正 or docs を hook 除外（別タスク）。

**Q4（responsive: `w-[` / `grid-cols-[4-9]` warn）→ 見送り（データで net-negative と判定）❌**

実データ計測の結論として、Q4 の2案はどちらも単一クラス matcher では false positive が許容外:

| 案 | 計測結果 | 判定 |
|---|---|---|
| `grid-cols-[4-9]` warn | 高列数グリッド8件中**6件がレスポンシブ接頭辞付き**（`md:grid-cols-4`×3, `sm:grid-cols-6`, `md:grid-cols-8`, `lg:grid-cols-5`）。matcher は variant を剥がすので**正しいレスポンシブグリッドを75%誤爆** | 見送り |
| `w-[` warn | 14件すべて modal/dropdown の正規コンポーネント幅（`w-[320px]`〜`w-[600px]`）。固定ページ幅と区別する閾値が prefix で表現不能 | 見送り |

→ responsive smell の本質（**接頭辞の有無・幅の閾値・要素文脈**）は variant を剥がす単一クラス検出の射程外。これは D3-01 が言う「合成・文脈次元」そのもの。**Q4 は S2(cheerio + variant 認識) に統合する**のが正しく、Quick win として無理に enforced 層に降ろすと正規コードを罰する。

### Quick wins 最終状態

| # | 状態 | 備考 |
|---|---|---|
| Q1 | ✅ 既存 | tsx/jsx 対応 + lint-core 一本化（①'） |
| Q2 | ✅ | arbitrary 値（色・影・font） |
| Q3 | ✅ 既存 | gradient / purple |
| Q4 | ❌ 見送り | データで net-negative → S2 統合 |
| Q5 | ✅ | html-attr 3件（tabindex / th scope / native date） |
| Q6 | ✅ | inline style hardcode color / `<style>` block |

**S1（生成物 CI gate）は実装済みと判明**（`.github/workflows/design-check.yml` の "Lint Generated UI" step が変更 `.html/.tsx/.jsx/.vue` を `design:lint-generated` にかけ error で exit 1）。よって Q2/Q5 の error ルールは既に CI ゲートで強制される。次の structural は **S2（合成 lint + Q4 統合 + quoted-`>` 解消）** が筋。

### 2026-05-30 — S2 着手: 合成 lint の土台 + スライス1（ネスト modal）✅

「製品として melta を強くする」方針で、Quick win の射程外（単一 class マッチでは届かない**合成・関係次元**）に着手。監査 D4-01 / section E が melta の差別化の本丸と位置づけた領域。

**土台（新しい検出次元）**:
- パーサ: **node-html-parser**（cheerio でなく）を選択。**推移依存ゼロ**で repo の依存極小ポリシーに合致（`private:true` なので consumer bloat も無い）。脆弱性持ち込みなし（既存の qs 等7件は MCP SDK 由来の pre-existing で無関係）。
- 新 detector **`composition`** + spec `compositionCheck` を追加（`types.ts` / `rule.schema.json` / `server.ts` / `validate.ts` の enum 同期）。
- 新モジュール `src/utils/composition-lint.ts`（`lintComposition(html)`）。`lint-generated.ts` が **`.html` のみ**に対して class/attr lint に加えて呼ぶ（JSX/.tsx の合成は AST が要る別物＝S4 送り）。

**スライス1: ネスト modal（`MODAL_NO_NESTED` 蘇生）**:
- 従来 `detector:manual` で死んでいた error ルールを `composition` + `{kind:"nested-selector", selector:"[role=\"dialog\"]"}` で活性化。
- **「子孫ネストだけ」を違反**にする設計。`[role=dialog]` が兄弟として複数並ぶだけ（正規のショーケース、docs に5個）は祖先を辿って非検知。
- **誤爆ゼロ実証**: docs/index.html（dialog×5 兄弟）= ネスト0 / ベンチ生成物 = ネスト0 / crafted 子孫ネスト = error 検知。single/double quote 混在もパーサが正規化。
- テスト: `tests/composition-lint.spec.ts`（検知4 + ガード3）。全 74 unit test green / tsc / design:check / drift 緑。

**S2 残（段階導入）**:
- スライス2: **画面内 distinct 色相数 > 3 → warn**（rubric「1画面3色」の機械化、Codex 警告どおり warn 始め）
- スライス3: **primary CTA 数 / カードネスト深さ → warn**（閾値は実データ較正）
- Q4 統合: responsive の固定幅・desktop-only grid を variant 認識込みでここに載せる
- quoted-`>` 解消: `tag-missing-attr`(Q5) をパーサベースに移せば自然解消

**既知のノイズ（別タスク）**: 生成物 docs/index.html に Q6 の inline-style hardcode color が104件 warn（showcase 自動生成由来）。warn かつ CI は変更ファイルのみなので無害だが、ジェネレータをトークン化 or docs を lint 除外すると消える。

**Codex review 対応（push 前）**:
- ✅ `anthropic.ts` の detector enum 同期漏れ修正（`composition` + `tailwind-class-segment` が抜けてた → `server.ts` と一致させた）
- ✅ 回帰テスト追加: 3階層ネストの集約挙動（1違反＋件数 token）/ コメント内 role=dialog 非検知 / selector の JSON round-trip。全77 test green
- 📋 **次タスク（benchmark 配線）**: `design/benchmarks/score.ts` は detector を `tailwind-class`/`prefix` だけ自前フィルタしており、**segment(Q3)/html-attr(Q5)/composition(S2) を全てスコアに反映していない**（pre-existing drift を S2 が広げた）。benchmark は手動実行のみで CI 非接続なので gate 影響はゼロだが、「ハーネスの効果を測る」には score.ts を lint-core/composition-lint 経由に寄せる必要がある。これは S7（benchmark rubric 配線）と同じ筋。
- 設計メモ（slice2 前）: `CompositionCheck` union は現状 `nested-selector` のみ。色相数/CTA数は `{kind:"count-threshold", selector, max}` を**新 kind として追加**する（`nested-selector` に threshold を後付けして混濁させない）。Codex 推奨。

### 2026-05-30 — S2 スライス2: ネスト interactive ✅（色数 lint は見送り）

当初「画面内 distinct 色相数 > 3 → warn」を予定したが、**実データで色数 lint も死んだ**（Q4 と同じ構造的理由）。代わりに同じ `nested-selector` 機構で**インタラクティブ要素の入れ子**を検出する方向にピボット。

**色数 lint が成立しない実証**:
- ベンチ生成物の distinct 色相数を計測（neutral 系を畳み込み）: 正規の良い出力が **5色相**（neutral + primary + status の amber/emerald/red）。D4 の崩壊リポロは 6色相（purple/pink）。
- → 「色数 > 3」は**セマンティックな status 色（正当）と装飾的乱用を数で区別できない**。良い設計が普通に5に届くので閾値3-4は誤爆、6で切ると D4 すら拾えない。距離が近すぎる。
- そもそも装飾色の本丸 purple/violet/fuchsia は **Q3(AI_NO_DECORATIVE_PURPLE) が既に捕捉**。残る status 色は意味があり量で罰せない。
- **教訓**: 合成のうち**count 系（色数・CTA数）は semantic/decorative を数で分離できず lint 化困難**。クリーンな合成 lint は **relational/structural**（ネスト・要素間関係）に集約される。これが section E の「合成は subtle」の正体。

**スライス2: ネスト interactive（`A11Y_NO_NESTED_INTERACTIVE`, error）**:
- `button`/`a`/`role=button` の中に別のインタラクティブ要素 = **HTML 仕様違反 + キーボード/SR/クリック判定が壊れる実バグ**。曖昧さゼロ。
- 新 kind 不要。`nested-selector` の selector をカンマ群 `a[href], button, [role="button"]` にするだけ（node-html-parser はカンマセレクタ + 属性セレクタ対応、`hasMatchingAncestor` がそのまま効く）。`a` でなく `a[href]` に絞り、href 無しの非インタラクティブ anchor を誤検知しない（Codex 指摘）。
- **ゼロFP実証**: docs/index.html は**インタラクティブ要素 240 個中ネスト0** / ベンチ生成物も0。crafted `a>button` は error 検知。
- テスト: `tests/composition-lint.spec.ts` に検知3 + ガード2（兄弟・非インタラクティブ子）。ルール 98→99、drift 同期、全 82 test green。

**S2 の現在地**: 合成 lint = relational 検出（ネスト modal / ネスト interactive）が2本。count 系は構造的に断念。次の relational 候補は「form の中の form」「カードネスト深さ（"card" の定義が要る）」等。閾値較正が要る count 系は据え置き。
