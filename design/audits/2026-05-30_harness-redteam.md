...
**S1（生成物 CI gate）は実装済みと判明**（`.github/workflows/design-check.yml` の "Lint Generated UI" step が変更 `.html/.tsx/.jsx/.vue` を `design:lint-generated` にかけ error で exit 1）。よって Q2/Q5 の error ルールは既に CI ゲートで強制される。次の structural は **S2（cheerio: 合成 lint + Q4 統合 + quoted-`>` 解消）** が筋。
