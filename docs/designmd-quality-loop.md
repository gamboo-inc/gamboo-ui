...
gamboo UI で 2026-07-03 に実走した内容の言語化。DESIGN.md を持つ（持ちたい）どの DS チームにも適用できる。
...
gamboo UI の運用はこれを構造的に防ぐ:

1. SSOT は契約（tokens.json / component 契約）。DESIGN.md は **generator（gamboo では `export-designmd.ts`）で導出**し、冒頭に「Generated — do not edit by hand」を明記。
