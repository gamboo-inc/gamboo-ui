# Gamboo Loop Playbook

> This document defines how Gamboo uses loops, pipelines, and scheduled checks.
...
Gamboo does not treat every repeated workflow as an agent loop.
...
`check_html.passed` means only "lint-clean draft". It does not prove Gamboo
...
path, for example `.gamboo-loop/runs.jsonl`.
...
| Action | Intentionally generate Gamboo-breaking UI, run checks, identify escapes |
